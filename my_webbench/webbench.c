 ...
 * Initial version of HTTP - a simple client-sever request-response, telenet-friendly protocol
 * Request nature:signle-line (method + path for requeted document)
...
webbench_t webbench = {
  ...
  .method = METHOD_GET,
  .proxyhost = NULL,
  .proxyport = 80  //it is the casual port for http. Note this is the client side. the connection function will send hand shake to the sever and check the connection.
};
 ...
 * flag specifieds how result are retuned for a logn option.If flag is NULL,  then  getopt_long()  re‐
...
int main(int argc, char** argv)
{
  ...
  while((opt=getopt_long(argc, argv, "912Vfrt:p:c:?h", long_options, &options_index)) != EOF)
  {
    ...
    {
      case 0: break;
      case 'f': webbench.force = 1;break;
      case 'r': webbench.force_reload = 1; break;
      case '9': webbench.http_type = HTTP09; break;
      case '1': webbench.http_type = HTTP10; break;
      case '2': webbench.http_type = HTTP11; break;
      case 'V': printf("This is Jack Shi's webbench\n. The Version is :"); printf(PROGARM_VERSION"\n");exit(0);
      case 't': webbench.benchtime=atoi(optarg); break;   //optarg is the global var provide the current ortarg
      case ':':
      case 'h':
      case '?': usage();return 2;break;
      case 'c': webbench.clients = atoi(optarg);break;
      /*proxy server parsing server:port format*/
      case 'p':
        tmp = strrchr(optarg, ':');
        webbench.proxyhost = optarg; /*The argument after the option is saved in the optarg variables.*/
        if(tmp == NULL)
          break;
        if(tmp == optarg)
        {
    ...
    /*In this project the web address should append at the end of the cmd. Hence, optind should less then argc*/
    ...
    if(webbench.clients == 0) webbench.clients = 1;
    if(webbench.benchtime == 0) webbench.benchtime = 60;
    ...
    printf("\n");
    if(webbench.clients == 1) printf("1 client");
    else printf("%d clients", webbench.clients);

    printf(", running %d sec", webbench.benchtime);
    if(webbench.force) printf(", early socket close");
    if(webbench.proxyhost != NULL) printf(", via proxy sever %s:%d\n", webbench.proxyhost, webbench.proxyport);
    if(webbench.force_reload) printf(", force_reload");
    printf(".\n");
    return bench();
...
static void build_request(const char* url)
{
  ...
  if(webbench.force_reload && webbench.proxyhost != NULL && webbench.http_type < HTTP10)
  {
  ...
  switch(webbench.method)
  {
    case METHOD_GET: strcpy(webbench.request, "GET"); break;
    case METHOD_HEAD: strcpy(webbench.request, "HEAD"); break;
    case METHOD_OPTIONS: strcpy(webbench.request, "OPTIONS");break;
    case METHOD_TRACE: strcpy(webbench.request, "TRACE"); break;
    default:
  ...
  if(webbench.proxyhost==NULL)
  {
       ...
       index(url+i, ':') < index(url+i, '/') )
    {
      ...
      if(webbench.proxyport == 0) webbench.proxyport = 80;
    }
    else {
  ...
  if(webbench.http_type > HTTP09) strcat(webbench.request, "\r\n");
}
...
static int bench(void)
{
  ...
  i = Socket(webbench.proxyhost == NULL?webbench.host:webbench.proxyhost, webbench.proxyport);
  ...
  /*
   * Step 1: one process generate one parent one child.the child pid is equal to 0 then go to sleep. the parent will go to the scecond around.
   * Step 2: the parent fork another child,child go to sleep and parent go back the the third around.
   * Step 3: same as above.
   * step 4: after 1 second all the child will wake up and break from the for loop.
   * */ 
  ...
  if(pid == (pid_t)0)
  {
    ...
    setvbuf(f, NULL, _IONBF, 0);  //send the data directly to other side of the pipe
    webbench.speed = 0;
    webbench.failed = 0;
    webbench.bytes = 0;
    ...
    while(1)
    {
      ...
      webbench.failed += j;
      webbench.bytes += k;
      if(--webbench.clients == 0) break;
    }
    fclose(f);

    printf("\nSpeed=%d pages/min, %d bytes/sec. \nRequests: %d succeed, %d failed.\n",
           (int)((webbench.speed + webbench.failed)/(webbench.benchtime/60.0f)),
           (int)(webbench.bytes/(float)webbench.benchtime),
            webbench.speed, webbench.failed);
  }
...
void benchcore(const char *host,const int port,const char *req)
{
  ...
  nexttry:while(1)
  {
    ...
    if(s < 0) {webbench.failed++;continue;} //if cannot connect with the server then mark as failed. 
    if(rlen!=write(s, req, rlen)) {webbench.failed++; close(s);continue;}
    if(webbench.http_type == HTTP09)
      if(shutdown(s,SHUT_WR)) {webbench.failed++; close(s);continue;}
    
    if(webbench.force == 0)
    {
      while(1)
      {
        if(webbench.timerexpired)break;
    ...
    if(close(s)) {webbench.failed++; continue;}
    webbench.speed++;
  }
}
